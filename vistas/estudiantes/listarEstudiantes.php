...
<section class="seccion-estudiantes">
  ...
  <div class="acciones-superiores">
    <a class="boton-nuevo" href="<?= BASE_URL ?>/index.php?modulo=estudiantes&accion=crear">➕ Nuevo estudiante</a>
  </div>
      ...
      <?php if ($estudiantes && $estudiantes->num_rows > 0): ?>
        <?php while ($row = $estudiantes->fetch_assoc()) : ?>
          <tr>
            ...
            <td>
              <a class="accion-editar" href="<?= BASE_URL ?>/index.php?modulo=estudiantes&accion=editar&id=<?= $row['id_estudiante'] ?>">Editar</a>
              <a class="accion-eliminar" href="<?= BASE_URL ?>/index.php?modulo=estudiantes&accion=eliminar&id=<?= $row['id_estudiante'] ?>" onclick="return confirm('¿Eliminar estudiante?')">Eliminar</a>
            </td>
          </tr>
        <?php endwhile; ?>
      <?php else: ?>
